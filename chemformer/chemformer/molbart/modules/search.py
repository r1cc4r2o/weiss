...
class Node:
    def __init__(self, model, x, vocabulary, device, data_device="cpu", batch_size=64, task='validation'):
        """
        ...
        with torch.no_grad():
            ...
            if isinstance(self.model, BARTweiss):
                if task == 'validation':
                    self.x = self.model.encode_validation(x).detach().permute(1, 0, 2)
                    self.x_mask = src_mask.detach().transpose(0, 1)
                elif task == 'test':
                    memory, memory_pad_mask = self.model.encode(x)
                    self.x = memory.detach().permute(1, 0, 2)
                    self.x_mask = memory_pad_mask.detach()
                else:
                    raise ValueError(f"Unknown task for [validation, test]: {task}")
            else:
...
class MultinomialNode(Node):
    def set_multinomial_width(self, multinomial_width):
        self.multinomial_width = multinomial_width

    def multinomial_sample(self, loglikelihood, num_samples=1):
        likelihood = torch.exp(loglikelihood)
        likelihood = torch.softmax(likelihood, dim=-1)
        next_chars = likelihood.multinomial(num_samples)
        loglikelihood = torch.gather(loglikelihood, dim=-1, index=next_chars)
        return loglikelihood, next_chars
    
    def _init_action(self, loglikelihood):
        # Perform the first step
        loglikelihood, next_chars = self.multinomial_sample(loglikelihood, self.multinomial_width)

        self.loglikelihood = loglikelihood.view(-1, 1)
        next_chars = next_chars.view(-1, 1)

        self.y = self.y.view(len(self.y), 1, -1).repeat(1, self.multinomial_width, 1).view(-1, 1)
        self.x = self.x[:, None].repeat(1, self.multinomial_width, 1, 1).view((-1,) + tuple(self.x.shape[1:]))
        self.x_mask = self.x_mask[:, None].repeat(1, self.multinomial_width, 1).view((-1,) + tuple(self.x_mask.shape[1:]))
    ...
    def action(self, next_loglikelhihood):
        if self.pos == 0:
            ...
            ll, idx = self.multinomial_sample(ll)
...
def multinomialsearch(node, multinomialsize, stop_criterion):
    node.set_multinomial_width(multinomialsize)
    print("Sampling with multinomial size: " + str(multinomialsize))
