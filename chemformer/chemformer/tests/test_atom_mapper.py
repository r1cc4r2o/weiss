...
from molbart.modules.retrosynthesis.disconnection_aware.utils import (
...
@pytest.mark.parametrize(
    ("reactants_smiles", "bond_atom_inds", "expected"),
    [
        ("[Cl:2].[CH:1]1=[CH:7][CH:6]=[CH:5][CH:4]=[CH:3]1", [1, 2], True),
        (
            "[Cl:2].[CH:1]1=[CH:7][CH:6]=[CH:5][CH:4]=[CH:3]1",
            [6, 7],
            False,
        ...
        (
            "[Cl:6].[CH:7]1=[CH:1][CH:2]=[CH:5][CH:4]=[CH:3]1",
            [1, 2],
            False,
        ),
        (
            "[Cl:6].[CH:1]1=[CH:7][CH:2]=[CH:5][CH:4]=[CH:3]1",
            [1, 2],
            True,
        ),
    ],
)
def test_verify_disconnection(reactants_smiles, bond_atom_inds, expected):
    assert verify_disconnection(reactants_smiles, bond_atom_inds) == expected
