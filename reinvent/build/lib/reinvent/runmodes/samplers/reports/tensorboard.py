...
def setup_TBData(sampled: SampleBatch, time: int, **kwargs):
    fraction_valid_smiles, fraction_unique_molecules, time, additional_report = report_setup(
        sampled, time, **kwargs
    )
    ...
    image_tensor, _ = make_grid_image(top_sampled_smiles, labels, ROWS * COLS, ROWS)

    return TBData(
        fraction_valid_smiles, fraction_unique_molecules, time, additional_report, image_tensor
    )
...
def write_report(reporter, data: TBData) -> None:
    ...
    reporter.add_text(
        "Data",
        f"Valid SMILES: {data.fraction_valid_smiles}%  "
    ...
    if data.image_tensor is not None:
        reporter.add_image(f"Most Frequent Molecules", data.image_tensor)
