...
from reinvent.models.transformer.core.dto.batch_dto import BatchDTO, BatchDTOWithZ
...
class PairedDatasetWithZ(tud.Dataset):
    """Dataset that takes a list of (input, output) pairs."""
    ...
    def __init__(
        ...
        z_t_s_enc, z_s_s_enc = enc_z
        
        for input_smi, output_smi, tanimoto, z_t_s, z_s_s in zip(
            smiles_input, smiles_output, tanimoto_similarities, z_t_s_enc, z_s_s_enc
        ):
            ok_input, ok_output = True, True
            try:
                tokenized_input = self._tokenizer.tokenize(input_smi)
    ...
    @staticmethod
    def collate_fn(encoded_pairs) -> BatchDTOWithZ:
        """Turns a list of encoded pairs (input, target) of sequences and turns them into two batches.
        ...
        encoded_inputs, encoded_targets, tanimoto_similarities, z = list(zip(*encoded_pairs))
        collated_arr_source, src_mask = _mask_batch(encoded_inputs)
        collated_arr_target, trg_mask = _mask_batch(encoded_targets)
