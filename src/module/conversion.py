...
class Conversions:
    ...
    def convert_to_standardized_smiles(self, smiles: str, fail_silently: bool = True) -> Optional[str]:
    ...
    def batched_convert_to_standardized_smiles(self, smiles: List[str], fail_silently: bool = True) -> List[str]:
        """Standardize SMILES for Mol2Mol
        ...
        param fail_silently: if True, raise an exception on error else, return None
        return: list of SMILES strings
        """
        return [self.convert_to_standardized_smiles(smi, fail_silently=fail_silently) for smi in tqdm(smiles)]
    ...
    def get_from_mol_morgan_fp_as_bit_vector(self, mol: Mol, radius: int = 2, n_bits: int = 1024) -> UIntSparseIntVect:
        """Get Morgan fingerprint as bit vector."""
        return AllChem.GetMorganFingerprintAsBitVect(mol, radius, nBits=n_bits)
    
    def get_from_smiles_morgan_fp_as_bit_vector(self, smiles: str, radius: int = 2, n_bits: int = 1024) -> UIntSparseIntVect:
        """Get Morgan fingerprint as bit vector."""

        # Convert to standardized SMILES
        std_smile = self.convert_to_standardized_smiles(smiles)
        # Convert to Mol object
        mol = MolFromSmiles(std_smile)
