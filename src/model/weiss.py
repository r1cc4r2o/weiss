# This file contains the implementation of Mol2MolWEISS and Mol2MolWEISSWithB
# Mol2MolWEISS is the original implementation of WEISS: Wasserstein Efficient 
# Sampling Strategy for LLMs in Drug Designs
...
class Mol2MolWEISS(torch.nn.Module):
    def __init__(
        self,
        *,
        vocabulary_size: int,
        ...
        super().__init__()
        """ Original implementation of Mol2MolWEISS.
    ...
    def get_z(self, src: torch.Tensor, src_mask: torch.Tensor, trg: torch.Tensor, trg_mask: torch.Tensor):
        ...
        Returns:
            ...
            torch.Tensor: the latent variable z_0.
    ...
    def encode(self, src: torch.Tensor, src_mask: torch.Tensor):
        ...
        src_emb = self.pe(self.token_emb(src))
        src_enc = self.transformer.encoder(src_emb, src_m)
        src_enc = src_enc + (self.inv_proj(z).unsqueeze(1) - self.inv_proj(torch.zeros_like(z)).unsqueeze(1))
        return src_enc, z
    ...
    def forward(self, src: torch.Tensor, src_mask: torch.Tensor, trg: torch.Tensor, trg_mask: torch.Tensor, z: List = None):
        ...
        Args:   
            ...
            trg (torch.Tensor): the target tensor.
            trg_mask (torch.Tensor): the target mask tensor.
            z (torch.Tensor): the latent variable z = [z_t_s, z_s_s].
            
        Returns:
            torch.Tensor: the decoded target tensor.
            
        """
        assert z is not None, "z = [z_t_s, z_s_s] must be provided"
        z_t_s, z_s_s = z
        ...
        # Enrich the source tensor with the latent variable z
        
        src_enc = src_enc + (self.inv_proj(z_t_s).unsqueeze(1) - self.inv_proj(z_s_s).unsqueeze(1)) 
